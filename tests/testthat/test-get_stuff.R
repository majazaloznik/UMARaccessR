...
test_that("new get functions work correctly", {
  with_mock_db({
    ...
    expect_equal(result, "Prispevek za pokojninsko in invalidsko zavarovanje iz drugih pravnih razmerij -- Mesečno")
    result <- sql_get_series_name_from_series_code(con, "SURS--1701102S--orig--C[skd]--M", schema = "test_platform")
    expect_equal(result, "Originalni podatki -- C PREDELOVALNE DEJAVNOSTI")
    ...
    result <- sql_get_table_id_from_table_code(con, "NR-BDP", schema = "test_platform")
    ...
    result <- sql_get_last_publication_date_from_table_id(361L, con, schema = "test_platform")
